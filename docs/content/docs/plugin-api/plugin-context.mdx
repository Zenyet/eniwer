...
`PluginContext` is the interface passed to `activate()`. It provides access to configuration, UI, AI, storage, and more.
...
ui: {
  ...
  showSavedAIResult(data: { title: string; content: string; ... }): void;
};
```
...
ai: {
  ...
  ): Promise<{ success: boolean; result?: string; thinking?: string; error?: string }>;
...
Each plugin gets isolated storage scoped by its `id`:
